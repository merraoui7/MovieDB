...
public class SearchingOP {
    ...
    public class Searcher extends AsyncTask<Void,Void,Void> {
        ...
        protected Void doInBackground(Void... voids) {
            list.clear();
            if (!url.equals(url1)&&!url.equals(url2)&&!url.equals(url3)){
                        ...
                        for (int i = 0 ; i < results.length() ; i++ ){
                            switch (media_type) {
                                case "movie":
                                    ...
                                    list.add(new Search(title, imgurl, type,rating,desc,id));
                                    break;
                                case "TV":
                                ...
                                case "people":
                                    ...
                                    id = results.getJSONObject(i).getInt("id");
                                    rating ="Popularity:"+ String.valueOf(results.getJSONObject(i).getDouble("popularity"));
                                    desc = "";
                                    list.add(new Search(title, imgurl, type,rating,desc,id));
                                    break;
                            }
